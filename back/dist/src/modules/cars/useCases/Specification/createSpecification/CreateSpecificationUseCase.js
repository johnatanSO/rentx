...
let CreateSpecificationUseCase = class CreateSpecificationUseCase {
    ...
    async execute({ name, description, }) {
        const alreadyExistSpecification = await this.specificationsRepository.findByName(name);
        if (alreadyExistSpecification)
            throw new AppError_1.AppError('Já existe uma especificação com este nome');
